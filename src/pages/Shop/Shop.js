    ...
    <div className="catalog">
        ...
        <div className='block1'>
            <div className='imgmast'><img src={'https://0fx825678.urest.org/__scale/uploads/s/x/i/p/xip56oooiblq/img/autocrop/ad665770e254a4105453bf101b4baa53.jpg?quality=60&width=1366&webp=1'} alt=''/></div>
                <div className='texth'><h1>Кованые изделия</h1></div>
                  <div className='textp'>
                      <p>Наши кованые изделия отличаются изысканным дизайном, высоким качеством и неповторимым стилем.
                        Каждое изделие создано с любовью и вниманием к деталям, что делает его по-настоящему уникальным.
                        Погрузитесь в мир красоты и изыска с нашими коваными изделиями и добавьте неповторимый штрих в ваш дом.</p>
                  </div>
        </div>
      </div>
      <div className="shop-container">
        <div className="category-list">
          <ul>
          <li className={selectedCategory === 'all' ? 'active' : ''} onClick={() =>
             handleTypeClick('all')}><p>Все изделия</p></li>
            {types.map((type) => (  <li key={type.id} className={selectedCategory === type.id ? 'active' : ''} 
            onClick={() => handleTypeClick(type.id)}><p>{type.name}</p></li>))}
          </ul>
        </div>
